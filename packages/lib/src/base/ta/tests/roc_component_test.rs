...
mod tests {
    use crate::base::{
        ...
        ta::roc_component::RateOfChangeComponent,
    };

    fn format_path(path: &str) -> String {
        format!("base/ta/tests/fixtures/roc/{}", path)
    }

    fn _test(
        cctx: &mut ComponentContext,
        target: &mut RateOfChangeComponent,
        expected: &[Option<f64>],
    ) {
    ...
    fn length_3_close() {
        ...
        _test(
            &mut ctx.clone(),
            &mut RateOfChangeComponent::new(ctx.clone(), 3),
            &expected,
        );
    }
    ...
    fn length_365_close() {
        ...
        _test(
            &mut ctx.clone(),
            &mut RateOfChangeComponent::new(ctx.clone(), 365),
            &expected,
        );
    }
}
