...
mod tests {
    ...
    fn _test(
        ...
        target_cross_over: &mut RecursiveCrossOver,
        ...
        for cctx in cctx {
            ...
            let output = target_cross_over.next(output_rsi.rsi, Some(30.0));
    ...
    fn test_crossover_with_rsi_btc_1d_length14_close() {
        let (_df, ctx, expected) = load_test_artifact_with_target(
            "implicit/recursive/cross_over/rsi/btc_1d_length_14_close.csv",
        );
        _test(
            &mut ctx.clone(),
            &mut RecursiveCrossOver::new(ctx.clone()),
