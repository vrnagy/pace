use crate::utils::math::{find_max_index, find_min_index};
...
pub fn compute_highest(series: &[Option<f64>], length: usize) -> Option<f64> {
    let index = find_max_index(series);
    return series[index];
}

pub fn compute_lowest(series: &[Option<f64>], length: usize) -> Option<f64> {
    let index = find_min_index(series);
    return series[index];
}
